...
async def process_chunk(chunk, start_idx, total_records, model_client, termination_condition):
    ...
    處理單一批次資料：
      ...
      - 組出提示，要求各代理人根據該批次資料進行分析，
        並給出各種遊戲分類對於好評率與價格深入分析。
      - 請 MultimodalWebSurfer 代理人利用外部網站搜尋功能，
        搜尋在這份資料中綜合數據較高的遊戲類型，並給出10個最符合本類型的遊戲，
        並將搜尋結果納入建議中。
      - 收集所有回覆訊息並返回。
    """
    # 將資料轉成 dict 格式
    ...
    prompt = (
        ...
        f"以下為該批次資料:\n{chunk_data}\n\n"
        "請根據以上資料進行分析，並提供當前價格與好評率的綜合推薦遊戲前10名，並最後分析哪種遊戲模式的好評率是相對高且多。"
        "其中請特別注意：\n"
        "  1. 分析各種遊戲分類對於好評率與價格；\n"
        "  2. 請 MultimodalWebSurfer 代理人利用外部網站搜尋功能，搜尋在這份資料中綜合數據較高的遊戲類型，並給出10個最符合本類型的遊戲，\n"
        "     並將搜尋結果整合進回覆中；\n"
        "  3. 最後請提供具體的建議和相關參考資訊。\n"
